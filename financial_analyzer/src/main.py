...
def run(
    ...
    try:
        ...
        dt_s = pd.to_datetime(processed["date"], errors="coerce")
        if hasattr(dt_s, "dt") and getattr(dt_s.dt, "tz", None) is not None:
            dt_s = dt_s.dt.tz_convert(None)
        processed = processed.assign(date=dt_s)

        # Detect signals
        logger.info("Detecting signals...")
        golden_dates = detect_golden_crossover(processed)
        death_dates = detect_death_cross(processed)
        ...
        # Save to DB
        engine = get_engine()
        save_daily_metrics(processed, engine=engine)
        ...
        logger.error("Failed to analyze stock %s: %s", ticker, exc)
